...
class MarketplaceSourceTests(unittest.TestCase):
    ...
    def test_tracked_unpublished_crawler_is_accepted_without_registration(self) -> None:
        self.manifest(self.root / "plugins/insane-crawl")
        _ = self.git("add", "plugins/insane-crawl")
        self.assert_cli_status(0)

    def test_unpublished_crawler_registration_is_rejected(self) -> None:
        crawler = self.root / "plugins/insane-crawl"
        self.manifest(crawler)
        manifest = crawler / ".claude-plugin/plugin.json"
        _ = manifest.write_text(json.dumps({
            "name": "insane-crawl", "version": "1.0.0", "description": "Fixture",
            "author": {"name": "Fixture"}, "license": "MIT",
        }), encoding="utf-8")
        marketplace = self.root / ".claude-plugin/marketplace.json"
        _ = marketplace.write_text(json.dumps({
            "name": "fixture", "owner": {"name": "Fixture"},
            "plugins": [
                {"name": "demo", "source": "./plugins/demo"},
                {"name": "insane-crawl", "source": "./plugins/insane-crawl"},
            ],
        }), encoding="utf-8")
        self.assert_cli_status(1)

    def test_other_tracked_unregistered_source_is_rejected(self) -> None:
        self.manifest(self.root / "plugins/other")
        _ = self.git("add", "plugins/other")
        self.assert_cli_status(1)
